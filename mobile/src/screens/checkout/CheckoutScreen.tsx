...
import { AppText, Button, Card, ErrorState, Loading, NoticeStrip, Screen } from '@/components/ui';
...
export default function CheckoutScreen({
  ...
  useEffect(() => {
    if (quote.data && paymentMethod === PaymentMethod.COD && !quote.data.codAllowed) {
  ...
  const placeOrder = useCallback(async (): Promise<void> => {
    ...
    setError(null);
    try {
      const result = await api.post<{ order: OrderDetailDto; requiresPayment: boolean }>(
        '/orders',
        {
          addressId,
          paymentMethod,
      ...
      if (err instanceof ApiRequestError) {
        ...
        if (err.code === ErrorCode.PRICE_CHANGED || err.code === ErrorCode.ITEM_OUT_OF_STOCK) {
          await quote.refetch();
  ...
  }, [addressId, quote, placing, paymentMethod, idempotencyKey, queryClient, onPlaced]);

  if (addresses.isLoading) return <Loading label="Loading checkout…" />;
  ...
  const selected = addresses.data?.find((item) => item.id === addressId) ?? null;
  ...
  return (
    <Screen>
          ...
          return (
            <View key={item.key} style={styles.stepItem}>
              <View
                style={[
                  styles.stepDot,
                  (done || active) && { backgroundColor: colors.primary },
                ]}
              >
                <AppText
                  variant="caption"
                  color={done || active ? colors.onPrimary : colors.textSecondary}
                >
                  {done ? '✓' : index + 1}
                </AppText>
              </View>
              <AppText variant="caption" color={active ? colors.primary : colors.textSecondary}>
                {item.label}
              </AppText>
            </View>
          );
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.base, paddingBottom: 180 }}>
          ...
          <View>
            ...
            {(addresses.data ?? []).length === 0 ? (
              ...
              (addresses.data ?? []).map((address) => (
                <Pressable
                  key={address.id}
                  onPress={() => address.isServiceable && setAddressId(address.id)}
                  style={[
                    styles.addressCard,
                    address.id === addressId && { borderColor: colors.primary },
                    !address.isServiceable && { opacity: 0.55 },
                  ]}
                >
            ...
            {quote.data?.serviceability.serviceable && (
              <Card style={{ marginTop: spacing.base, backgroundColor: colors.primarySurface }}>
                <AppText variant="bodyStrong" color={colors.primary}>
                  Delivery in{' '}
                  {formatEtaRange(quote.data.etaMinMinutes, quote.data.etaMaxMinutes)}
                </AppText>
              </Card>
            )}
          </View>
        ...
        {step === 'payment' && (
          <View>
            ...
            {/* COD is shown DISABLED WITH A REASON rather than hidden — a
                missing option is confusing, an explained one is not. */}
            <PaymentOption
              label="Cash on Delivery"
              hint={
                quote.data?.codAllowed
                  ? 'Pay when your order is delivered'
                  : (quote.data?.codBlockedReason ?? 'Not available for this order')
              }
        ...
        {step === 'review' && bill && (
          <View>
            ...
            <Card style={{ marginTop: spacing.base }}>
              ...
              <Row label={`Item Total (${bill.itemCount} items)`} value={formatPaise(bill.itemsSubtotalPaise)} />
              ...
              {bill.platformFeePaise > 0 && (
                <Row label="Platform Fee" value={formatPaise(bill.platformFeePaise)} />
              )}
            ...
            <Card style={{ marginTop: spacing.base }}>
              <AppText variant="bodyStrong">
                {paymentMethod === PaymentMethod.COD ? 'Cash on Delivery' : 'Pay by UPI'}
              </AppText>
            </Card>
          </View>
        )}
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + spacing.base }]}>
        ...
        {step === 'review' ? (
          <Button
            label={paymentMethod === PaymentMethod.COD ? 'Place Order' : 'Continue to Payment'}
          ...
          <Button
            ...
            disabled={step === 'address' ? !addressId || !quote.data?.serviceability.serviceable : false}
          />
        )}
      </View>
    </Screen>
  ...
  return (
    ...
    >
      <View style={{ flex: 1 }}>
        ...
        <AppText variant="caption" color={disabled ? colors.danger : colors.textSecondary}>
          {hint}
        </AppText>
      </View>
