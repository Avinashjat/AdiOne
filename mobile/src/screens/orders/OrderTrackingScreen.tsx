...
export default function OrderTrackingScreen({
  ...
  const cancelOrder = useMutation({
    mutationFn: (reason: string) => api.post(`/orders/${orderId}/cancel`, { reason }),
    onSuccess: () => {
      setCancelError(null);
    ...
    onError: (err: Error) =>
      setCancelError(
        err instanceof ApiRequestError ? err.message : 'Could not cancel this order.',
      ),
  });
  ...
  useOrderSocket({
    onStatusChanged: (event) => {
      if (event.orderId === orderId) void queryClient.invalidateQueries({ queryKey: keys.order(orderId) });
    },
  });

  useEffect(() => {
    if (!isLive) void queryClient.invalidateQueries({ queryKey: keys.orders });
  }, [isLive, queryClient]);

  if (isLoading) return <Loading label="Loading your order…" />;
  if (isError || !order) {
    return <ErrorState message="We could not load this order." onRetry={() => void refetch()} />;
  }

  const exception = TERMINAL_ORDER_STATUSES.includes(order.status) &&
  ...
  return (
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.base, paddingBottom: insets.bottom + spacing.xxl }}>
        <Card style={{ backgroundColor: colors.primarySurface }}>
          <View style={styles.summaryRow}>
            <View>
              <AppText variant="caption" color={colors.textSecondary}>
                {isLive ? 'Estimated delivery' : 'Order'}
              </AppText>
              <AppText variant="display" color={colors.primary}>
                {isLive && order.etaMinutes ? `${order.etaMinutes} mins` : order.statusLabel}
              </AppText>
            </View>
                ...
                return (
                  <View key={entry.step} style={styles.timelineRow}>
                    ...
                    <View style={{ flex: 1, paddingBottom: spacing.lg }}>
                      <AppText variant="bodyStrong" color={done || active ? colors.textPrimary : colors.textMuted}>
                        {entry.label}
                      </AppText>
                      {entry.at && (
                        <AppText variant="caption" color={colors.textSecondary}>
                          {formatDateTimeInZone(new Date(entry.at), 'Asia/Kolkata')}
                        </AppText>
                      )}
                      {active && (
        ...
        <Card style={{ marginTop: spacing.base }}>
          ...
          {order.items.map((item) => (
            <View key={item.id} style={styles.itemRow}>
              ...
              <AppText variant="body">{formatPaise(item.lineTotalPaise)}</AppText>
            </View>
          ))}
          ...
          <AppText variant="caption" color={colors.textSecondary}>
            Paid by {order.paymentMethod === 'COD' ? 'Cash on Delivery' : 'Online'} ·{' '}
            {order.paymentStatus}
          </AppText>
        </Card>
            ...
            onPress={() =>
              Alert.alert('Cancel this order?', 'This cannot be undone.', [
                { text: 'Keep order', style: 'cancel' },
                {
                  text: 'Cancel order',
                  style: 'destructive',
                  // A reason is required by the API and is what the store sees
                  // on its board, so it is sent rather than left blank.
